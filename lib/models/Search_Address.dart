...
import '../../Api_Servicess.dart'; // Путь к твоему AddressApiService
...
class _SearchAddressScreenState extends State<SearchAddressScreen> {
  ...
  // --- Ближайшие / исторические адреса (для примера подгрузки) ---
  ...
  // Подтверждение выбора и передача данных назад на экран карты
            ...
            Expanded(
              child: SingleChildScrollView(
                ...
                child: Column(
                  ...
                  children: [
                    // --- ЕДИНЫЙ БЛОК ВВОДА (В СТИЛЕ ЯНДЕКСА) ---
                    Container(
                    ...
                    // --- ЕДИНЫЙ СПИСОК РЕЗУЛЬТАТОВ (РЕЗУЛЬТАТЫ ПОИСКА + НИЖЕ БЛИЖАЙШИЕ АДРЕСА) ---
                    if (_addressSuggestions.isNotEmpty) ...[
                      // 1. Секция результатов поиска
                      Padding(
                      ...
                      // 2. Секция «Ближайшие адреса» (в том же длинном списке при прокрутке ниже)
                      const Padding(
                        padding: const EdgeInsets.symmetric(horizontal: 4.0, vertical: 6.0),
                        child: Text(
                          'Ближайшие адреса',
                      ...
                      Container(
                        ...
                        child: ListView.separated(
                          shrinkWrap: true,
                          physics: const NeverScrollableScrollPhysics(),
                          // Для примера выводим элементы или заглушку, если список пуст
                          itemCount: _addressSuggestions.length > 3 ? 3 : _addressSuggestions.length,
                          separatorBuilder: (_, __) => const Divider(height: 1, color: Color(0xFFF3F4F6)),
                          itemBuilder: (context, index) {
                            // Берем элементы для блока "ближайшие" (например, дублируем или выводим релевантные)
                            final item = _addressSuggestions[index];
                            final String addressName = item['name'] ?? item['street'] ?? '';
                      ...
                      Container(
                        ...
                        child: Column(
                          ...
                          children: [
                            ...
                            const SizedBox(height: 8),
                            Text(
                              '${_selectedTown!['name']}, ${_selectedAddress!['name']}',
