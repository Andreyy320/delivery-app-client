...
/// Экран меню ресторана с категориями и корзиной по shopId
          ...
          IconButton(
            ...
            onPressed: () {
              Navigator.push(
                context,
                MaterialPageRoute(
                  builder: (_) => CartScreen(shopId: widget.shopId,  restaurantName: widget.restaurantName, // 🔹 прокидываем имя ресторана
                  ),
                ),
              ).then((_) => setState(() {}));
            },
              ...
              child: Column(
                children: _categories.map((category) {
                  final items = widget.menu.where((e) {
                    final matchesCategory = e.category == category;
...
/// Карточка блюда с кнопкой добавления в корзину
class DishCardWithStatus extends StatelessWidget {
  ...
  Widget build(BuildContext context) {
    ...
    final cartNotifier = getCart(userId, shopId); // теперь 2 аргумента
